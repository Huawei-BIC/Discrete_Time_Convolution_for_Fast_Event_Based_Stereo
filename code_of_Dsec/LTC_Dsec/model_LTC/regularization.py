...
class ExpansionBlock3d(nn.Module):
    ...
    def forward(self, block_input, shortcut_from_contraction):
        ...
        if output_of_upsampling.size(2) != shortcut_from_contraction.size(2):
            b,c,d,h,w = output_of_upsampling.size()
            shortcut_from_contraction = F.interpolate(shortcut_from_contraction,(d,h,w),mode='nearest')
