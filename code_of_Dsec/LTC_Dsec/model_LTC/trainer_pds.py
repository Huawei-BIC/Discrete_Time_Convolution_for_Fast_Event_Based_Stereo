...
def _move_tensors_to_cuda(dictionary_of_tensors):
    ...
    if isinstance(dictionary_of_tensors, list):
        # print(dictionary_of_tensors)
...
class Trainer(object):
    def __init__(self, parameters):
        ...
        self._test_errors = []
        self._number_of_examples_to_visualize = 0
    ...
    def load_checkpoint(self, filename, load_only_network=False):
        ...
        #     k = k[7:]
        #     checkpoint_buf[k] = v
        # self._network.load_state_dict(checkpoint_buf)
    ...
    def _train_for_epoch(self,epoch_idx):
        ...
        Duration = 0
        cgvl = []
        transform = transforms.Compose([transforms.RandomCrop(448,576,0.5)])
        for batch_index, batch in enumerate(self._training_set_loader):
            # if batch_index == 1:
            #     break
            if self._DA:
                sample = {}
                sample['left'], sample['right'], sample['disp'] = batch['left']['event_queue'],batch['right']['event_queue'],batch['left']['disparity_image']
                sample = transform(sample)
                batch['left']['event_queue'],batch['right']['event_queue'],batch['left']['disparity_image'] = sample['left'], sample['right'], sample['disp']
            ...
            self._network.module._temporal_aggregation._LTC_Conv.apply_weight_constraints()
            self._current_losses.append(batch['loss'])
            # cgvl.append([self._network._temporal_aggregation._LTC_Conv.tau_m.clone().detach().cpu().numpy()])
