...
class Regularization(nn.Module):
    ...
    def __init__(self, number_of_features=8):
        ...
        self._upsample_to_halfsize = \
            network_blocks.transposed_convolutional_block_4x4x4_stride_211(
                number_of_features, number_of_features // 2)
        self._upsample_to_fullsize = \
            network_blocks.transposed_convolution_3x4x4_stride_122(
                number_of_features // 2, 1)

        # self.SEblock = network_blocks.SELayer(32)
        # self.eca_block_contract = network_blocks.eca_block(32)
        # self.eca_block = network_blocks.eca_block(128)
    ...
    def forward(self, matching_signatures, shortcut_from_left_image):
        ...
        # output = self.eca_block_contract(output.squeeze(2)).unsqueeze(2)
        
        del shortcut
        for expansion_block in self._expansion_blocks:
            output = expansion_block(output, shortcuts_from_contraction.pop())

        full_size_output = self._upsample_to_fullsize(self._upsample_to_halfsize(output)).squeeze_(1)
        # full_size_output = self.eca_block(full_size_output)
        # full_size_output = self.SEblock(full_size_output)
