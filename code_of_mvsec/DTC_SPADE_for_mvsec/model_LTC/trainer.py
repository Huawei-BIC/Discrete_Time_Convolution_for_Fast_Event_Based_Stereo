...
class Trainer(pds_trainer.PdsTrainer):
    ...
    def _run_network(self, batch_or_example):
        ...
        batch_or_example['network_output'] = self._network(batch_or_example)
    ...
    def _compute_error(self, example):
        ...
        # print(estimated_disparity.shape)
         ...
         #   visualization.save_image(
          #      filename=self._left_events_template.format(example_index + 1),
           #     image=left_events_visualization,
            ...
            #np.save('before_spade%s.npy'%example_index, example['before_spade'].clone().cpu().numpy())
            #np.save('after_spade%s.npy'%example_index, example['after_spade'].clone().cpu().numpy())
...
def initialize_optimizer(stereo_network, main_lr, temporal_aggregation_lr):
    if isinstance(stereo_network, network.ShallowEventStereo):
        ...
        return optim.RMSprop(
            ...
            # {
            #     "params": stereo_network.stay_embedding.parameters()    
            # },
            # {
            #     "params": stereo_network.conv1.parameters()
            # },
            # {
            #     "params": stereo_network.conv2.parameters()
            # },
            {
                "params": stereo_network.spade.parameters()    
            },
            {
                "params": stereo_network._matching.parameters()
            }, {
